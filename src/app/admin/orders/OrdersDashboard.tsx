    ...
    <div className="container-page py-10 sm:py-14">
      <div className="flex flex-col gap-4 border-b border-oven-cream/10 pb-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          disabled={loggingOut}
          className="inline-flex h-fit items-center gap-2 self-start rounded-full border border-oven-cream/15 px-5 py-2.5 text-sm font-semibold text-oven-cream/80 transition-colors hover:border-oven-flame hover:text-oven-flame-light disabled:opacity-60"
        >
        ...
        {orders.length === 0 ? (
          ...
          <ul className="space-y-4">
            {orders.map((order) => (
              <li
                key={order.id}
                className="rounded-xl2 border border-oven-cream/10 bg-oven-teal-deep/40 p-5 shadow-card"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col items-end gap-2">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-wide ${STATUS_STYLES[order.status]}`}
                    >
                      {order.status}
                    </span>
                    <select
