...
export default function CartView({ branches }: { branches: Branch[] }) {
  ...
  const [orderConfirmation, setOrderConfirmation] = useState<{ id: string; total: number } | null>(
    null
  );
    ...
    return (
      <div className="container-page flex min-h-[60vh] flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-3 max-w-md text-oven-cream/70">
          Order <span className="font-mono text-oven-crust">#{orderConfirmation.id.slice(-8)}</span>{" "}
          for <span className="font-mono text-oven-crust">{formatRs(orderConfirmation.total)}</span> has
          been received. We&apos;ll call you shortly to confirm.
        </p>
  ...
  if (lines.length === 0) {
    return (
      <div className="container-page flex min-h-[60vh] flex-col items-center justify-center py-24 text-center">
        ...
        <p className="mt-3 max-w-md text-oven-cream/70">
          Browse the menu and add a few things — pizzas, burgers, shawarma, drinks or a student
      ...
      <div className="no-print">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="mt-4 font-display text-3xl text-oven-cream sm:text-4xl">
              ...
              {lines.reduce((n, l) => n + l.quantity, 0) === 1 ? "" : "s"} in your cart
            </h1>
          </div>
          <button
            type="button"
            onClick={clearCart}
            className="text-sm font-medium text-oven-cream/50 underline-offset-4 hover:text-red-400 hover:underline"
          >
            Clear cart
          </button>
        </div>
              ...
              <li key={line.lineId} className="flex flex-wrap items-center gap-4 p-5 sm:p-6">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-full border border-oven-cream/20 text-oven-cream hover:border-oven-flame-light hover:text-oven-flame-light"
                  >
                    −
                  </button>
                ...
                <button
                  ...
                  className="ml-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-oven-cream/40 hover:bg-red-500/10 hover:text-red-400"
                >
        ...
        <div className="mt-8 ml-auto max-w-sm rounded-xl2 border border-oven-cream/10 bg-oven-teal-deep/50 p-6 shadow-card">
          ...
          {discountPercent > 0 ? (
            <div className="mt-3 flex items-center justify-between text-sm text-oven-flame-light">
              ...
              <span className="font-mono">− {formatRs(discountAmount)}</span>
            </div>
          ) : null}

          <div className="mt-4 flex items-center justify-between border-t border-oven-cream/10 pt-4 text-lg font-semibold text-oven-cream">
            <span>Total</span>
            <span className="font-mono text-oven-flame-light">{formatRs(total)}</span>
          </div>

          <button
            type="button"
            onClick={() => window.print()}
            className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-full bg-flame-gradient px-6 py-3 text-sm font-semibold text-oven-charcoal shadow-ember transition-transform hover:scale-[1.01]"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <path
        ...
        <div className="mt-8 ml-auto max-w-sm rounded-xl2 border border-oven-flame/25 bg-oven-teal-deep/50 p-6 shadow-card">
          ...
          <p className="mt-1 text-sm text-oven-cream/60">
            We&apos;ll call you to confirm before preparing your order.
          </p>
          ...
          <form onSubmit={handlePlaceOrder} noValidate className="mt-5 space-y-4">
            ...
            <div>
              ...
              {checkoutErrors.customerName ? (
                <p className="mt-1.5 text-sm text-red-400">{checkoutErrors.customerName}</p>
              ) : null}
            </div>
            ...
            <div>
              ...
              <input
                ...
                aria-invalid={Boolean(checkoutErrors.customerPhone)}
                className="w-full rounded-lg border border-oven-cream/15 bg-oven-charcoal/60 px-4 py-2.5 text-oven-cream placeholder:text-oven-cream/30 focus:border-oven-flame-light"
                placeholder="0300-1234567"
              />
              {checkoutErrors.customerPhone ? (
                <p className="mt-1.5 text-sm text-red-400">{checkoutErrors.customerPhone}</p>
              ) : null}
            </div>
            ...
            <div>
              ...
              <input
                ...
                aria-invalid={Boolean(checkoutErrors.customerEmail)}
                className="w-full rounded-lg border border-oven-cream/15 bg-oven-charcoal/60 px-4 py-2.5 text-oven-cream placeholder:text-oven-cream/30 focus:border-oven-flame-light"
                placeholder="you@example.com"
              />
              {checkoutErrors.customerEmail ? (
                <p className="mt-1.5 text-sm text-red-400">{checkoutErrors.customerEmail}</p>
              ) : null}
            </div>
            ...
            {branches.length > 0 ? (
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-oven-cream/15 bg-oven-charcoal/60 px-4 py-2.5 text-oven-cream focus:border-oven-flame-light"
                >
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Delivery address, gate code, extra spicy…"
              />
            </div>
            ...
            <button
              ...
              className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-flame-gradient px-6 py-3 text-sm font-semibold text-oven-charcoal shadow-ember transition-transform hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-60"
            >
            ...
            <div role="status" aria-live="polite">
              {checkoutMessage ? (
                <p className={`text-sm ${checkoutStatus === "error" ? "text-red-400" : "text-oven-cream/70"}`}>
                  {checkoutMessage}
                </p>
              ) : null}
            </div>
