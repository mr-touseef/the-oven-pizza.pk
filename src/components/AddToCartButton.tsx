...
export default function AddToCartButton({
  ...
  const confirmTitle = item.sizeLabel
    ? `Add ${quantity} × ${item.name} (${item.sizeLabel})?`
    : `Add ${quantity} × ${item.name}?`;

  if (confirmOpen) {
    return (
      <div className="flex w-full flex-col gap-2 rounded-xl2 border border-oven-flame/30 bg-oven-flame/5 p-2.5">
        <p className="text-xs font-medium leading-snug text-oven-crust">{confirmTitle}</p>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => setConfirmOpen(false)}
            className="flex-1 rounded-xl2 border border-oven-teal/20 bg-white/70 px-2.5 py-1.5 text-xs font-medium text-oven-crust/70 transition-colors hover:bg-oven-teal/10"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 rounded-xl2 bg-flame-gradient px-2.5 py-1.5 text-xs font-semibold text-oven-charcoal shadow-ember"
          >
            Yes, add it
          </button>
        </div>
